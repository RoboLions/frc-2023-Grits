...
public class ManualState extends State{
    ...
    transitions.add(new Transition(() -> {
        ...
        public void execute() {
            ...
            if (translationVal < 0) {
            RobotMap.elevator.manualDrive(RobotMap.elevator.applyDeadband(-1.0 * translationVal) * 0.5);
            } else {
                RobotMap.elevator.manualDrive(RobotMap.elevator.applyDeadband(-1.0 * translationVal) * 0.25);
            }
        }
