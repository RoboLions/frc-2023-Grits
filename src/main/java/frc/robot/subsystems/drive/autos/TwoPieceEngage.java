...
public class TwoPieceEngage extends AutoModeBase {
    
    // trajectory action
    TrajectoryAction driveToIntake;
    TrajectoryAction driveToScore;
    TrajectoryAction driveToEngage;
    ...
    public TwoPieceEngage() {
        ...
        ArrayList<PathPlannerTrajectory> twoPieceEngage = (ArrayList<PathPlannerTrajectory>) PathPlanner.loadPathGroup(
            "Two Piece + Engage", 
            new PathConstraints(3.0, 2.0)
        );

        initialHolonomicPose = twoPieceEngage.get(0).getInitialHolonomicPose();

        driveToIntake = new TrajectoryAction(
            twoPieceEngage.get(0), 
        ...
        driveToEngage = new TrajectoryAction(
            twoPieceEngage.get(2),
            RobotMap.swerve::getPose, 
            Constants.SWERVE.SWERVE_KINEMATICS, 
            Constants.SWERVE.Profile.X_CONTROLLER,
            Constants.SWERVE.Profile.Y_CONTROLLER,
